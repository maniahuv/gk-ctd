...
Type* compileLValue(void) {
  ...
  if (varType != NULL && varType->typeClass == TP_ARRAY) {
...
void compileForSt(void) {
  ...
  eat(SB_ASSIGN);
  expr1Type = compileExpression();
  checkTypeEquality(var->varAttrs->type, expr1Type);
  eat(KW_TO);
  expr2Type = compileExpression();
  checkTypeEquality(var->varAttrs->type, expr2Type);
  eat(KW_DO);
  compileStatement();
}
